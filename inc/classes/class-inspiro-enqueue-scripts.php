...
if (! class_exists('Inspiro_Enqueue_Scripts')) {
	...
	class Inspiro_Enqueue_Scripts
	{
		...
		public function enqueue_scripts()
		{
			...
			// Theme stylesheet.
			wp_enqueue_style('style', inspiro_get_assets_uri('style', 'css'), array(), LOGOVO_THEME_VERSION);
			wp_enqueue_style('logovo-style', inspiro_get_assets_uri('logovo', 'css'), array(), LOGOVO_THEME_VERSION);

			// Load the dark colorscheme.
			if ('dark' === inspiro_get_theme_mod('colorscheme') || is_customize_preview()) {
				wp_enqueue_style('inspiro-colors-dark', inspiro_get_assets_uri('colors-dark', 'css'), array('logovo-style'), LOGOVO_THEME_VERSION);
			}

			wp_enqueue_script('inspiro-lite-js-plugins', inspiro_get_assets_uri('plugins', 'js'), array('jquery'), LOGOVO_THEME_VERSION, true);

			wp_enqueue_script('inspiro-lite-script', inspiro_get_assets_uri('scripts', 'js'), array(
				'inspiro-lite-js-plugins',
				'jquery'
			), LOGOVO_THEME_VERSION, true);
			...
			wp_add_inline_style('logovo-style', $theme_css_data);
		}
		...
		{
			// Block styles.
			wp_enqueue_style('inspiro-block-editor-style', inspiro_get_assets_uri('editor-style', 'css'), array(), LOGOVO_THEME_VERSION);
